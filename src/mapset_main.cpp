...
string to_lowercase(const string& str) {
    auto lower_view = str | views::transform([](unsigned char c) { return tolower(c); });
    return { lower_view.begin(), lower_view.end() };
}

SetList<string> load_stopwords(istream& stopwords) {
    return SetList<string>{ranges::istream_view<string>(stopwords) | views::transform(to_lowercase)};
}
...
MapArray<string, int> count_words(istream& document, SetList<string>& stopwords) {
    auto words_view = ranges::istream_view<string>(document)
        | views::transform(to_lowercase) | views::filter([&](const string& s) { return !stopwords.contains(s); });
